...
func (s *Server) Routes() http.Handler {
	...
	mux.HandleFunc("GET /extension.mjs", s.handleExtensionBundle)
...
func (s *Server) handleExtensionBundle(w http.ResponseWriter, _ *http.Request) {
	bundlePath := strings.TrimSpace(os.Getenv("RELAY_EXTENSION_BUNDLE"))
	if bundlePath == "" {
		writeError(w, http.StatusNotFound, "extension bundle is not configured")
		return
	}
	bundle, err := os.ReadFile(bundlePath)
	if err != nil {
		writeError(w, http.StatusNotFound, "extension bundle is not available")
		return
	}
...
func renderInstallScript(httpURL, wsURL string) (string, error) {
	...
	extensionURL := strings.TrimRight(httpURL, "/") + "/extension.mjs"
	data := map[string]string{
		"RelayHTTPURLShell": shellQuote(httpURL),
		"RelayWSURLShell":   shellQuote(wsURL),
		"ExtensionURLShell": shellQuote(extensionURL),
