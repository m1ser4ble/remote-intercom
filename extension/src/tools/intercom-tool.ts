...
export interface RemoteIntercomTool {
  name: string;
  description: string;
  execute(input: RemoteIntercomToolInput): Promise<RemoteIntercomToolResult>;
  handle(input: RemoteIntercomToolInput): Promise<RemoteIntercomToolResult>;
  dispose(): void;
}
...
export function createRemoteIntercomTool(options: CreateRemoteIntercomToolOptions = {}): RemoteIntercomTool {
  ...
  const execute = async (input: RemoteIntercomToolInput): Promise<RemoteIntercomToolResult> => {
    switch (input.action) {
      case "connect": {
...
function installRelayEventHandlers(context: ToolContext): () => void {
  const disposeMessageHandler = context.client.on(RelayEventType.MessageSend, (event) => {
    ...
    void context.onNotify?.(`Device ${event.from ?? "unknown"} asks: ${payload.text}`, event);
  });

  const disposeJoinHandler = context.client.on(RelayEventType.JoinRequest, (event) => {
    const payload = event.payload as JoinRequestPayload | undefined;
    if (payload === undefined || typeof payload.joinRequestId !== "string") {
      return;
    }
    const channelName = context.channelName ?? event.channelId ?? "this channel";
    context.pending.addJoinRequest({
      id: payload.joinRequestId,
    ...
    void context.onNotify?.(`Device ${payload.deviceName} wants to join ${channelName}. Approve?`, event);
  });
